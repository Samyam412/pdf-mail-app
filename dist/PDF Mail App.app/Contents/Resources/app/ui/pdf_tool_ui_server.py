...
def default_workspace_root() -> Path:
    configured_root_raw = os.environ.get("PDF_MAIL_USER_ROOT", "").strip()
    if configured_root_raw:
        configured_root = Path(configured_root_raw).expanduser()
        return configured_root.resolve()

    if ROOT_DIR.name == "app" and ROOT_DIR.parent.name == "Resources":
...
class PDFToolHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/api/pick-source-folder":
            ...
            initial_path = resolve_user_path(initial) if initial else default_workspace_root()
            try:
